    ...
    <section id="services" className="py-16 sm:py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="reveal mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 sm:mt-4 text-sm sm:text-base text-foreground/70">
            From first sketch to launch and beyond — we handle every pixel and line of code.
          </p>
        </div>

        <div className="mt-10 sm:mt-16 grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 stagger">
          {services.map((s, i) => (
            <div
              key={s.title}
              className="reveal reveal-zoom card-tilt group relative overflow-hidden rounded-2xl border border-border bg-surface p-5 sm:p-7 transition-smooth hover:border-primary/40"
            ...
            >
              ...
              <div className="grid h-11 w-11 sm:h-12 sm:w-12 place-items-center rounded-xl bg-primary/10 text-primary transition-smooth group-hover:bg-primary group-hover:text-primary-foreground group-hover:scale-110">
                <s.icon size={20} className="sm:w-[22px] sm:h-[22px]" />
              </div>
              <h3 className="mt-4 sm:mt-5 text-base sm:text-lg font-bold text-foreground">{s.title}</h3>
              <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm leading-relaxed text-foreground/70">{s.desc}</p>
              <ArrowUpRight
                size={16}
